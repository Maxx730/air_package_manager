...
onready var _cash_indicator = get_node("shade/top/shade/border/content/vert/items/hor/cash")
...
# actions for opening the different action buttons above
func _open_shop():
	_ui._close_modals()
...
# end of action button methods

# methods for next/pre arrows
...
func _next_aircraft():
	...
	_ui._determine_ui_actions(_aircraft._state)
	_aircraft._update_transit_ui()
# end of next/pre methods

# determination methods that need to be run when loading/purchasing/selling aircraft
func _determine_aircraft_actions(state):
	match state:
		_globals.PLANE_STATE.LANDED:
			_shop.disabled = false
			_location.disabled = false
			_customize.disabled = false
			_depart.disabled = false
		_globals.PLANE_STATE.IN_TRANSIT:
			_shop.disabled = false
			_location.disabled = true
			_customize.disabled = true
			_depart.disabled = true
		-1:
			pass
	
	_determine_switcher_content()
...
func _open_locations():
	...
	_globals._get_aircraft()._state = _globals.PLANE_STATE.DEPARTING
	_globals._map._determine_aircraft_scene(_globals._get_aircraft()._state)
	_ui._determine_ui_actions(_globals._get_aircraft()._state)
	_ui._depart_tutorial.visible = false
	_ui._location_switcher.visible = true
	var _locat = _globals._locations[_globals._get_aircraft()._location]
	_globals._main_camera._target = _locat
	_ui._location_switcher._update_switcher_info(_globals._get_aircraft()._location)
...
func _aircraft_depart():
	...
	var _length = floor(_distance / _aircraft._max_speed)
